...
class MarketAPI {
	...
	private function updatePricesFromEVECentralXML() {
		...
		// Get the timestamp of the last 5-minute increment.
	 	$datetime = $this->db->getLastTimestamp(time(),300);
	
		// Query DB for all systemIDs and typeIDs.
		$systemIDs = $this->db->getAllSystemIDs();
		$typeIDs = $this->db->getAllTypeIDs();
		...
		return $this->db->storePriceRecords($priceData);
	}
